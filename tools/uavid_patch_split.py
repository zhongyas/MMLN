...
def padifneeded(image, mask):#将图像的长宽都变为1024的倍数
...
def patch_format(inp):
    ...
    for img_path, mask_path in zip(img_paths, mask_paths):
        ...
        img=cv2.resize(img.copy(), (4096, 2048))
        mask=cv2.resize(mask.copy(), (4096, 2048))
...
if __name__ == "__main__":
    ...
    t0 = time.time()
    mpp.Pool(processes=mp.cpu_count()).map(patch_format, inp)#作用是将inp中的每一个元素作为参数传递给patch_format函数
    t1 = time.time()
    split_time = t1 - t0
