...
ORIGIN_IMG_SIZE = (512, 512)#原始图像大小
INPUT_IMG_SIZE = (512, 512)#输入图像大小    
TEST_IMG_SIZE = (512, 512)#测试图像大小
...
def train_aug(img, mask):#对图像进行数据增强
...
def get_val_transform():#对图像进行归一化
    val_transform = [
        albu.Normalize()
    ]
...
class VaihingenDataset(Dataset):#定义数据集
    ...
    def __getitem__(self, index):
        ...
        img = torch.from_numpy(img).permute(2, 0, 1).float()#将图像转换为tensor
    ...
    def get_img_ids(self, data_root, img_dir, mask_dir):#获取图像id
    ...
    def load_img_and_mask(self, index):#加载图像和mask
        img_id = self.img_ids[index]
    ...
    def load_mosaic_img_and_mask(self, index):#加载马赛克图像和mask
...
def show_img_mask_seg(seg_path, img_path, mask_path, start_seg_index):#展示图像、mask和预测的mask
...
def show_seg(seg_path, img_path, start_seg_index):#展示图像和mask
    seg_list = os.listdir(seg_path)
    seg_list = [f for f in seg_list if f.endswith('.png')]
...
def show_mask(img, mask, img_id):#展示图像和mask
