...
class QYUnetFormerLoss(nn.Module):
...
class QYUnetFormerLoss_s(nn.Module):
    ...
    def forward(self, logits, labels):
        if self.training and len(logits) == 2:
            logit_main, logit_aux = logits
            loss = self.main_loss(logit_main, labels) + 0.3 * self.aux_loss(logit_aux, labels)
        else:
            loss = self.main_loss(logits, labels)

        return loss
    
class QYUnetFormerLoss_b(nn.Module):
    ...
    def forward(self, logits, labels):
        if self.training and len(logits) == 2:
            logit_main, logit_aux = logits
            loss = self.main_loss(logit_main, labels) + 0.5 * self.aux_loss(logit_aux, labels)
        else:
            loss = self.main_loss(logits, labels)
